...
ltn_semestre = tratatpf(ltn_semestre)
...
tabela_ntn = pd.concat(
    [ntnb_ontem, 
    ntnb_semana_anterior,
    ntnb_semestre]
)
...
grafico_ntnb = (
    p9.ggplot(tabela_ntn) + 
    p9.aes(x = "vencimento", y = "taxa", color = "data_negociacao") +
    p9.geom_line() + 
    p9.labs(
        x = "",
        y = "",
        title = "ETTJ - Juro Real",
        caption = "Elaborado por DIC-RF"
    ) + 
    p9.theme(
        plot_title = p9.element_text(weight="bold"),
    ...
    p9.scale_x_date(date_labels="%Y-%m") + 
    p9.coord_cartesian(ylim=(6.5,10)) + 
    p9.scale_color_manual(
        values = ["red", "blue", "orange"]
    )
)
...
tabela_implicita = (
    ...
    .filter(
        ["data_negociacao", "vencimento", "inflacao_implicita"],
        axis = "columns"
...
(
    p9.ggplot(tabela_implicita) + 
    p9.aes(x = "vencimento", y = "inflacao_implicita", color = "data_negociacao") + 
    p9.geom_line() + 
    p9.labs(
        x = "",
        y = "",
